...
const QuizMonitorPanel = ({ quiz, course, onBack }) => {
  ...
  const handleUploadCSV = async () => {
    ...
    try {
      await useMonitorStore.getState().uploadCSV(
        course.course_id,
        result?.questionpaper_id || quiz.id, // Use questionpaper_id if available, else quiz.id
        marksCSV
      );
      setMarksCSV(null);
      monitorQuiz(quiz.id, course.course_id, selectedAttempt.attempt_number);
  ...
  const handleExtendTime = async (paperId) => {
    ...
    // Re-fetch monitoring data to refresh UI
    monitorQuiz(quiz.id, course.course_id, selectedAttempt.attempt_number);
  };

  // Handle special attempt for a user
      ...
      {result && result.stats && (
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
          ...
          <div className="flex flex-row gap-2 md:flex-col md:gap-3 mt-2 md:mt-0">
            <button
              className="text-white px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg font-semibold bg-blue-500/80 active:scale-95 transition text-xs sm:text-sm whitespace-nowrap flex-shrink-0 hover:bg-blue-600 flex items-center gap-2"
            ...
            <button
              className="text-white px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg font-semibold bg-green-500/80 active:scale-95 transition text-xs sm:text-sm whitespace-nowrap flex-shrink-0 hover:bg-green-600 flex items-center gap-2"
              // onClick={handleShowStatistics} // Implement as needed
              disabled
            >
                  ...
                  <tbody>
                    {result.papers.map((paper, idx) => (
                      <tr key={paper.id} className="border-b border-white/10">
                        ...
                        <td className="px-3 py-2 text-center">0:0:0</td>
                        ...
                        <td className="px-3 py-2 text-center">
                          <div className="flex items-center gap-1 text-center justify-center">
                            <input
                              type="text"
                              className="w-16 px-2 py-1 rounded bg-black/30 border border-blue-500/20 text-green-400 font-semibold focus:outline-none focus:border-blue-400 transition"
